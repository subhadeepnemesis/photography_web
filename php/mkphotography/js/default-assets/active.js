(function ($) {
    ...
    if ($.fn.magnificPopup) {
        ...
        $(".hover-content").click(function(){
            ...
            if ( $(this).find("span").text() != "" ){
                span_data_array = $(this).find("span").text().split('_');
                var mfp = $.magnificPopup.instance;
                mfp.items = [];
                for (var i = 1; i <= span_data_array[2]; i++) {
                    var temp = { src: 'img/assets/'+span_data_array[0]+'/'+span_data_array[1]+'/'+i+'.jpg' };
                    mfp.items.push(temp);
                }
        ...
        var $mfp = $('.portfolio-img').magnificPopup({
            type: 'image',
            gallery: {
                enabled: true,
                preload: [0, 2],
    ...
    $('.testimonial').on('click', function () {
        ...
        $('#testimonial_title').val(ele.attr('id'));
        $('#testimonial_desc').text(ele.text());
    ...
    $('#frm_contact').on('submit', function (e) {
        ...
        var mb = $('#mob').val();
        if ( mb.length >= 10){
            var numObj = new Number(mb);
            var num = numObj.valueOf();
            if (num > 0 && typeof num == 'number')
                error = false;
            else
                $('#msg').html('Please enter valid mobile number.');
        }
        else
            $('#msg').html('Please enter 10 digit mobile number.');
        if (error)
            e.preventDefault();
    });
